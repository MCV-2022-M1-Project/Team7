...
class Measures(Metric):
    ...
    def compute_mae(self,mask1,mask2):
        ...
        mask1 = Measures.mask_normalize(mask1)
        mask2 = Measures.mask_normalize(mask2)
        sumError = np.sum(np.absolute((mask1.astype(float) - mask2.astype(float))))
        maeError = sumError/(float(h)*float(w)+1e-8)

        return maeError
    
    def compute_ave_MAE_of_methods(self,gt_name_list,rs_dir_lists):
        ...
        for i in range(0,num_gt):
            ...
            for j in range(0,num_rs_dir):
                ...
                try:
                    ...
                    continue
                try:
                    tmp_mae = Measures.compute_mae(gt,rs) # compute the mae
                except IOError:
    ...
    def compute_PRE_REC_FM_of_methods(self,gt_name_list,rs_dir_lists,beta=0.3):
        #input 'gt_name_list': ground truth name list
        #input 'rs_dir_lists': to-be-evaluated mask directories (not the file names, just folder names)
        ...
        for i in range(0,num_gt):
            print('>>Processed %d/%d'%(i+1,num_gt),end='\r')
            gt = io.imread(gt_name_list[i]) # read ground truth
            gt = Measures.mask_normalize(self,gt)*255.0 # convert gt to [0,255]
            ...
            for j in range(0,num_rs_dir):
                ...
                try:
                    ...
                    rs = Measures.mask_normalize(rs)*255.0 # convert rs to [0,255]
                except IOError:
                    #print('ERROR: Couldn\'t find the following file:',rs_dir_lists[j]+gt_name)
                    continue
                try:
                    pre, rec = Measures.compute_pre_rec(gt,rs,mybins=np.arange(0,256))
                except IOError:
                    #print('ERROR: Fails in compute_mae!')
                    continue
    ...
    def plot_save_pr_curves(self,PRE, REC, method_names, lineSylClr, linewidth, xrange=(0.0,1.0), yrange=(0.0,1.0), dataset_name = 'TEST', save_dir = './', save_fmt = 'jpg'):
